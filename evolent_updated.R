...
#Read specific column of original data and create data frame
beer_review <- data.frame(original_data$review_appearance,original_data$review_palette,original_data$review_overall,original_data$review_taste,original_data$review_aroma)
beer_id <- data.frame(original_data$beer_beerId)

#calculate mean of all review
beer_review_mean <- rowMeans(beer_review)
beer_id_review <-data.frame(beer_id,beer_review_mean)

#calculate mean of same brewerID
beer_id_review_ave <- setDT(beer_id_review)[,mean(beer_review_mean),by=beer_id]

#barplot of all result data
barplot(height = beer_id_review_ave$V1,names.arg = beer_id_review_ave$original_data.beer_beerId,
      xlab = 'brewer_ID', 
      ylab = 'Customer_Rating', 
      main = 'All Results', col = "green")

#Query for find top 3 strongest values

query = sqldf("select * from beer_id_review_ave where V1>= 5")
View(query)

barplot(height = query$V1,names.arg = query$original_data.beer_beerId,
        xlab = 'brewer_ID', 
        ylab = 'Customer_Rating', 
        main = 'Top 3 Breweries', col = "green")
